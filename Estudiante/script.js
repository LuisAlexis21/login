        ...
        examen.preguntas.forEach((pregunta, index) => {
            ...
            questionDiv.innerHTML = `
                ...
                ${pregunta.opciones.map((opcion, i) => `
                    <label>
                        <input type="radio" name="respuesta${index}" value="${opcion}">
                        ${opcion}
                    </label><br>
                `).join('')}
            `;
...
async function enviarRespuestas(id, examen) {
    ...
    examen.preguntas.forEach((pregunta, index) => {
        ...
        if (respuesta) {
            ...
            if (respuesta.value.toLowerCase() === pregunta.respuestaCorrecta.toLowerCase()) {
                puntosObtenidos++;
            }
        } else {
            respuestas.push(null); // Para respuestas vacías
        }
    });
